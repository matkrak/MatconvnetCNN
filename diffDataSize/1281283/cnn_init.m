...
net.layers{end+1} = struct('type', 'conv', ...  %1
                           'weights', {{f*randn(11,11,3,30, 'single'), zeros(1, 30, 'single')}}, ...
                           'stride', 1, ...
                           'pad', 0) ;
...
net.layers{end+1} = struct('type', 'conv', ...  %3
                           'weights', {{f*randn(9,9,30,60, 'single'),zeros(1,60,'single')}}, ...
                           'stride', 1, ...
                           'pad', 0) ;
...
net.layers{end+1} = struct('type', 'conv', ...  %5 (7)
                           'weights', {{f*randn(9,9,60,400, 'single'),  zeros(1,400,'single')}}, ...
                           'stride', 1, ...
                           'pad', 0) ;
net.layers{end+1} = struct('type', 'pool', ...  %4
                           'method', 'max', ...
...
net.layers{end+1} = struct('type', 'conv', ...  %7
                           'weights', {{f*randn(5,5,400,600, 'single'), zeros(1,600,'single')}}, ...
                           'stride', 1, ...
                           'pad', 0) ;
net.layers{end+1} = struct('type', 'relu') ;    %8
net.layers{end+1} = struct('type', 'conv', ...  %9
                           'weights', {{f*randn(4,4,600,2, 'single'), zeros(1,2,'single')}}, ...
                           'stride', 1, ...
                           'pad', 0) ;
