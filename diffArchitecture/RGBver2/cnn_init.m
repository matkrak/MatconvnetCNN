...
net.layers{end+1} = struct('type', 'conv', ...  %1
                           'weights', {{f*randn(15,15,3,20, 'single'), zeros(1, 20, 'single')}}, ...
                           'stride', 1, ...
                           'pad', 0) ;
...
net.layers{end+1} = struct('type', 'conv', ...  %3
                           'weights', {{f*randn(15,15,20,50, 'single'),zeros(1,50,'single')}}, ...
                           'stride', 1, ...
                           'pad', 0) ;
...
net.layers{end+1} = struct('type', 'conv', ...  %5 (7)
                           'weights', {{f*randn(9,9,50,200, 'single'),  zeros(1,200,'single')}}, ...
                           'stride', 1, ...
                           'pad', 0) ;
net.layers{end+1} = struct('type', 'pool', ...  %4
                           'method', 'max', ...
...
net.layers{end+1} = struct('type', 'conv', ...  %7
                           'weights', {{f*randn(6,6,200,500, 'single'), zeros(1,500,'single')}}, ...
                           'stride', 1, ...
                           'pad', 0) ;
net.layers{end+1} = struct('type', 'relu') ;    %8
net.layers{end+1} = struct('type', 'conv', ...  %9
                           'weights', {{f*randn(1,1,500,50, 'single'), zeros(1,50,'single')}}, ...
                           'stride', 1, ...
                           'pad', 0) ;
net.layers{end+1} = struct('type', 'conv', ...  %9
                           'weights', {{f*randn(1,1,50,2, 'single'), zeros(1,2,'single')}}, ...
                           'stride', 1, ...
                           'pad', 0) ;
net.layers{end+1} = struct('type', 'softmaxloss') ; %10
